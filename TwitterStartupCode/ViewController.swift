...
class ViewController: UIViewController {
    ...
    @IBAction func more(_ sender: AnyObject?) {
        ...
        let request = twitterRequest?.requestForOlder ?? ViewController.ucdRequest
    ...
    @IBAction func refresh(_ sender: AnyObject?) {
        // if request does not exist, create new one
        let request = twitterRequest?.requestForNewer ?? ViewController.ucdRequest
        DispatchQueue.global(qos: DispatchQoS.QoSClass.userInitiated).async {
